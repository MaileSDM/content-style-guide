...
const HeaderWrapper = styled(({ className, children }) => (
  <div className={`${className} ${"HeaderWrapper"} ${"HeaderWrapper_"}${config.metadata.short_name}`}>
    {children}
  </div>
))`
  ...
  border-bottom: 0px solid ${(props) => props.theme.header.border};
  padding: 10px 0;
  position: fixed;
  top:0;
  width:100%;
...
const Header = ({ setShowSearch, location, themeProvider, show, toggleFullscreenMode }) => (
  <StaticQuery
    ...
    render={(data) => {
      ...
      const logoImg = config.header.logo;
      const logoLink2 = logo.link2 !== '' ? logo.link2 : '/';
      const logoImg2 = config.header.logo2;
      ...
      return (
        <>
          ...
          <HeaderWrapper show={show}>
            <Logo link={logoLink} img={logoImg} title={headerTitle} link2={logoLink2} img2={logoImg2} />
